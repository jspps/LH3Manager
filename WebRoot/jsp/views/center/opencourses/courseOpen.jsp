...
<body class="body_bg1">
    ...
    <div class="user_content">
        ...
        <div class="kcszbtn">
        	<!-- <div class="fl reseach"><input type="text" class="r_txt f_mor"><input value="" class="r_btn" type="submit"></div>
         ...
         <div class="div_table f_st div_table_kc">
        	<table class="table" border="0" cellpadding="0" cellspacing="0">
                ...
                <tbody>
               <form action="center/modifyOpenKindCustByLhub" id="form_add">
               <input id="kindid_add" name="kindid" nullmsg="请选择套餐" type="hidden"/>
	               <tr id="tr_from_add">
	                   <td style="display:none;"><a href="javascript:void(0);" onclick="openWindow('add');" id="uu_val1_add">请选择</a></td>
	                   <td style="display:none;"><a href="javascript:void(0);" onclick="openWindow('add');" id="uu_val2_add">请选择</a></td>
	                   ...
	                   <td><input class="kc_text_2" type="text" name="uname" nullmsg="学生姓名不能为空"/></td>
	                   <td><input class="kc_text_2" type="text" name="uphone" zz="电话" nullmsg="学生手机号不能为空" /></td>
                    ...
                    <c:forEach items="${pageEnt.listPages}" var = "ent" varStatus="">
	                    <tr id="tr_date_${ent.id}">
	                    	<td style="display:none;">${ent.kindEn.nmLhub}</td>
	                    	<td style="display:none;">${ent.nmDepart}</td>
	                   ...
	                   <tr id="tr_from_${ent.id}" style="display: none">
	                    <form action="center/modifyOpenKindCustByLhub" id="form_${ent.id}">
	                    	<input name="okcustid"  value="${ent.id}" type="hidden"/>
                   			<input id="kindid_${ent.id}" value="${ent.kindid}"  name="kindid"  type="hidden"/>
		                    <td style="display:none;"> 
		                    	<a  href="javascript:void(0);"  onclick="openWindow('${ent.id}');" id="uu_val1_${ent.id}">${ent.kindEn.nmLhub}</a>
		                    </td>
	                        <td style="display:none;"> 
	                        	<a  href="javascript:void(0);" onclick="openWindow('${ent.id}');" id="uu_val2_${ent.id}">${ent.nmDepart}</a>
	                        </td>
        ...
        <p:pageTag name="pageEnt" action="admin/courseOpen"/>
    </div>	 
    </div>
    <i class="clear"></i>
    </div>
	...
	function delfromexamt(id){
			...
			jQuery.post("center/delCourseopened",{"okcustid":id}, function(data) {
	...
	function closeWindow(kindid,val1,val2,val3,val4,val5,val6,val7,uuid,agentitemid){
   ...
   function openWindow(uuid){
		    jQuery("#operate").attr("src","center/actingProject?courseOpen=1&uuid="+uuid);
			jQuery('#window').show();
			jQuery('#window').window({
				"title" : "选择套餐",
				width : jQuery("body").width()/1.5,
				height : jQuery("body").height()-300,
				modal : true,
				closable : true,
				minimizable : false,
				maximizable : false,
